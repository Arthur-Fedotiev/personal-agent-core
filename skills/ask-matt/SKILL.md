...
- **`/good-push`** — ship this iteration: find the checkout that actually holds this session's files, commit them there if they are still dirty, then an ordinary push. Ask only when the files straddle two checkouts, the tree mixes in other changes, or the change itself is unclear. Model-invoked on "good push" and "just push".
